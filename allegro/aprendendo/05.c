...
// antes de continuar eu quero criar um programa que eu possa mexer uma barra pra lá e pra cá e também girar
// depois eu quero poder fazer um lançamento com uma bolinha seguindo as leis da física
...
/*
PARA PODER FAZER AS COISAS SE MOVEREM:
precisaremos armazenar a posição e a velocidade de tudo e atualizar tudo continuamente.
*/

//para isso criamos:
enum BOUNCER_TYPE { //isso aqui serviu para enumerar todas as coisas que estão sendo animadas. Foi a enumeração
    BT_HELLO = 0,
    BT_MYSHA,
    BT_TRIANGLE,
    BT_RECTANGLE_1,
...
// e também:
typedef struct BOUNCER //definiu essa struct pq todas as coisas estao se movendo da mesma forma
{
    float x, y;
    float dx, dy;
    int type;
...
int main()
{
    ...
    BOUNCER obj[BT_N];
    for(int i = 0; i < BT_N; i++) //isso aqui é para atribuir uma posição aleatória para cada objeto a animar
    { //e também a velocidade que é o dx e o dy
        BOUNCER* b = &obj[i];
        b->x = rand() % 640;
        b->y = rand() % 480;
    ...
    while(1)
    {
        ...
        switch(event.type)
        {
            case ALLEGRO_EVENT_TIMER:
                for(int i = 0; i < BT_N; i++) //para que as coisas quiquem na borda e voltem
                {
                    BOUNCER* b = &obj[i];
                    b->x += b->dx;
                    b->y += b->dy;
